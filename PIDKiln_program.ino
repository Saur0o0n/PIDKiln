...
File dir,file;
  ...
  while((file=dir.openNextFile()) && Programs_DIR_size<=count){    // now we do acctual loading into memory
    char* fname,tmp[32]; uint8_t len2;
    ...
    if(!Programs_DIR_size) Programs_DIR[Programs_DIR_size].sel=1; // make first program seleted
    else Programs_DIR[Programs_DIR_size].sel=0;
...
void rotate_selected_program(int dir){
  for(uint16_t a=0; a<Programs_DIR_size; a++){
    if(Programs_DIR[a].sel>0){
      ...
      if(dir>0 && a<(Programs_DIR_size-1)){   // if we are going UP and there is more programs - do it, if it's the end of program list, do nothing
        Programs_DIR[a].sel=0;  // delete old selection
        Programs_DIR[a+1].sel=1;
        Program_sel=a+1;
        return;
